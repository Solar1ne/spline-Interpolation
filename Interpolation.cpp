...
void genData(double step, double a, double left, double right)
{
	n = (right - left) / step + 1;
	KnotArray = new knot[n];
	for (int i = 0; i < n; i++)
...
int main()
{
	...
	//if(Load_Data()!=-1)
	//{
	genData(0.1, 10.0, -1.05, 1.05);

	BuildSpline();
	//scanf("%lf",&x);
	for (double x = -1.0; x <= 1.0; x += 0.00001)
		//printf("x:  %10lf ans:   %lf\n",x,Interpolate(x));
		printf("err: %lf\n", fabs(Interpolate(x) - sin(10.0 * x)));
	//scanf("%lf",&x);
